...
fn main() {
    ...
    if f_or_d.is_dir() {
        ...
        let p = Parser::new(empty_string); // Fake parser for bootstrap code
        let mut c: CodeWriter = CodeWriter::new(p, "Sys", true); // Boostrap code calls the Sys init function
            ...
            glob(format!("{}/*.vm", filename).as_str()).expect("Failed to read glob pattern")
        {
            match files {
                Ok(path) => {
                    ...
                    let mut c = CodeWriter::new(p, path.file_name().unwrap().to_str().unwrap().split('.').nth(0).unwrap(), false);

                    println!("{:?} has more lines: {:?}", &file_contents, c.parser.has_more_lines());
        ...
        let mut c: CodeWriter = CodeWriter::new(p, &filename, true);
